...
def getpackages(num):
    ...
    for i in content:
        i = i.replace("\\xc3\\xa7", "&#xE7;").replace(
            "\\xc3\\xaa", "&#234;").replace("%23x27;", "&#39;")
        packagename = i[0].title() + i[1:]

        # Replace any spaces with %20
        i = i.replace(" ", "%20").replace("&amp;", "%26").replace("\\xc3\\xa7", "&#xE7;").replace("#", "%23").replace(
            "'", "&#39;").replace("\\xc3\\xaa", "&#234;").replace("&%23", "&#39;").replace("&#39;x27;", "'")
...
with open("packages.txt", 'w') as packages, open("README.md", 'w') as readme:
    ...
    packages.write("\nThere are a total of %s packages available as at %s\n\n" % (
        total, current_time))
    packages.write(
        "Instruction on how to contribute to this repository can be found in `contrib.txt`\n\n")
    readme.write("## %s \n\n" % (message))
    readme.write("\nThere are a total of `%s` packages available as at `%s`\n\n" % (
        total, current_time))
    readme.write(
